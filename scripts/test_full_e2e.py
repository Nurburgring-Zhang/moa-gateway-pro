...
capabilities = [
    ...
    # Wave 13 — 5 new capabilities
    ("tool-screening", {"tool_name": "exec", "arguments": {"cmd": "rm -rf /"}}),
    ("tool-screening", {"tool_name": "search", "arguments": {"query": "hello world"}}),
    ("anthropic-compat", {"action": "parse", "anthropic_request": {
        "model": "claude-3-5-sonnet", "max_tokens": 1024,
        "messages": [{"role": "user", "content": "Hello"}]}}),
    ("anthropic-compat", {"action": "format_sse", "delta": "Hello", "model": "claude-3-5-sonnet"}),
    ("token-bucket", {"action": "try_consume", "key": "e2e_test", "tokens": 1}),
    ("token-bucket", {"action": "state"}),
    ("request-dedup", {"action": "check", "method": "POST", "path": "/v1/chat",
                       "body": {"msg": "hi"}}),
    ("request-dedup", {"action": "record", "method": "POST", "path": "/v1/chat",
                       "body": {"msg": "hi"}, "response": {"answer": "hello"}}),
    ("request-dedup", {"action": "check", "method": "POST", "path": "/v1/chat",
                       "body": {"msg": "hi"}}),
    ("trace", {"action": "start"}),
    ("trace", {"action": "parse_traceparent",
