"""Helper: write mx_annot.py"""
import sys
from pathlib import Path

content = '''"""mx_annot - A-39 MX + A-40 fan-in + A-44 Moai-sidechain mx CLI
    ...
    for i, line in enumerate(text.splitlines(), start=1):
        ...
        if m:
            ...
            try:
                ...
                continue
            annotations.append(MXAnnotation(
                tag=tag, content=content, file_path=file_path,
                line_number=i, language=language,
            ))
    return annotations
...
'''
Path('moa_gateway/capability/mx_annot.py').write_text(content, encoding='utf-8')
print('written, size:', Path('moa_gateway/capability/mx_annot.py').stat().st_size)
