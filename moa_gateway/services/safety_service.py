...
def _load_prompt_canary():
    from ..capability.prompt_canary import inject, check
    return inject, check
...
def _load_frozen():
    from ..capability.frozen_zone import (
        is_frozen, add, is_evolvable, can_modify,
        assert_modifiable, list_sentinels,
    )
...
def _load_anthropic_compat():
    from ..capability.anthropic_compat import (
        parse_request, format_sse, format_response,
        format_tool_use, format_tool_result, format_error,
    )
    return (parse_request, format_sse, format_response,
            format_tool_use, format_tool_result, format_error)


def _load_llm_merge():
    from ..capability.llm_merge import merge, fallback
...
class SafetyService(ServiceBase):
    ...
    def _register_methods(self):
        self._methods["secret_scan"] = ServiceMethod(
            name="secret_scan", description="扫描路径中的 secret (admin only)",
            func=self.secret_scan,
            input_required=["path"],
        )
        self._methods["prompt_canary"] = ServiceMethod(
            name="prompt_canary", description="prompt canary (inject / check)",
        ...
        self._methods["tool_screening"] = ServiceMethod(
            name="tool_screening", description="筛选工具调用(防 rm -rf / 等危险)",
            func=self.tool_screening,
            input_required=["tool_name", "arguments"],
        )
        self._methods["output_wrapping"] = ServiceMethod(
            name="output_wrapping", description="输出包装(sanitize/wrap/unwrap/needs_wrapping)",
            func=self.output_wrapping,
            input_required=["action"],
            input_optional=["content", "source", "trust", "max_length", "wrapped"],
        )
        self._methods["frozen"] = ServiceMethod(
            name="frozen", description="frozen zone 管理 (is_frozen/add/is_evolvable/can_modify/assert_modifiable/list_sentinels)",
            func=self.frozen,
            input_required=["action"],
        )
        self._methods["grace"] = ServiceMethod(
            name="grace", description="grace window (register/should_block/status/warnings)",
            func=self.grace,
            input_required=["action"],
        )
        self._methods["anthropic_compat"] = ServiceMethod(
            name="anthropic_compat", description="Anthropic API 兼容(parse/format_sse/format_response/format_tool_*)",
            func=self.anthropic_compat,
            input_required=["action"],
        )
        self._methods["llm_merge"] = ServiceMethod(
            name="llm_merge", description="LLM 合并 (merge/fallback)",
            func=self.llm_merge,
            input_required=["action"],
        )
        self._methods["version"] = ServiceMethod(
            name="version", description="版本管理(add/get/latest/parse_rating/parse_battle/swap_positions_battle)",
            func=self.version,
            input_required=["action"],
        )
        self._methods["worktree"] = ServiceMethod(
            name="worktree", description="worktree 管理 (admin only)",
            func=self.worktree,
            input_required=["action"],
        )
    ...
    def frozen(self, action, path=None, zone=None, freeze=None, sentinel=None, reason=None, added_at=None):
        ...
        if action == "add":
            return add(path=path or "", zone=zone or "freeze", sentinel=sentinel or "",
                      reason=reason or "", added_at=added_at or 0.0)
    ...
    def anthropic_compat(self, action, **kwargs):
        ...
        parse_request, format_sse, format_response, format_tool_use, format_tool_result, format_error = fns
        if action == "parse":
        ...
        if action == "format_tool_use":
            return format_tool_use(tool_id=kwargs.get("tool_id", ""),
                                    name=kwargs.get("name", ""),
                                    input=kwargs.get("input", {}))
        if action == "format_tool_result":
            return format_tool_result(tool_use_id=kwargs.get("tool_use_id", ""),
                                      content=kwargs.get("content", ""),
                                      is_error=kwargs.get("is_error", False))
        if action == "format_error":
            return format_error(error_type=kwargs.get("error_type", ""),
    ...
    def version(self, action, **kwargs):
        ...
        if action == "add":
            return add(proposal_id=kwargs.get("proposal_id", ""),
                       content=kwargs.get("content", ""),
        ...
        if action == "swap_battle":
            return swap(judge_response=kwargs.get("judge_response", ""),
