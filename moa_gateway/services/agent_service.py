...
class AgentService(ServiceBase):
    ...
    def _register_methods(self):
        ...
        self._methods["bubble_should_continue"] = ServiceMethod(
            name="bubble_should_continue", description="检查 agent 是否继续",
            func=self.bubble_should_continue,
            input_required=["agent_id"],
        )
        self._methods["bubble_recent"] = ServiceMethod(
            name="bubble_recent", description="最近 bubble 事件",
            func=self.bubble_recent,
            input_required=["agent_id"], input_optional=["n"],
        )
    ...
    def acquire_with_wait(self, lock_id, session_id, timeout=10.0, retry_interval=0.01):
        mgr = _get_lock_manager()
        ok = mgr.acquire_with_wait(lock_id, session_id, timeout=timeout, retry_interval=retry_interval)
        return {"acquired": ok}
    ...
    def register_mcp(self, name, description="", parameters=None, returns="ok"):
        ...
        tool = MCPTool(name=name, description=description,
                        parameters=parameters or {}, handler=handler, returns=str(returns))
    ...
    def bubble_escalate(self, parent_id, agent_id, action_desc, reason):
        mgr = _get_bubble_mgr()
        return mgr.escalate(parent_id=parent_id, agent_id=agent_id,
    ...
    def bubble_schedule(self, event_id, event_type, agent_id, payload=None, timestamp=None):
        mgr = _get_bubble_mgr()
        return mgr.schedule_event(event_id=event_id, event_type=event_type,
                                  agent_id=agent_id, payload=payload or {},
