...
def _load_mx_annot():
    from ..capability.mx_annot import cli, fanin, parse

    return parse, fanin, cli


def _load_checkpoint():
    from ..capability.checkpoint import delete, list_all, load, save
...
class ConfigService(ServiceBase):
    ...
    def _register_methods(self):
        self._methods["config"] = ServiceMethod(
            name="config",
            description="配置 stack (get/set/unset/merge/permission)",
            func=self.config,
            input_required=["action"],
        )
        self._methods["mx"] = ServiceMethod(
            name="mx",
            description="mx annotation (parse/fanin/cli)",
            func=self.mx,
            input_required=["action"],
        )
        self._methods["checkpoint"] = ServiceMethod(
            name="checkpoint",
            description="checkpoint (admin only)",
            func=self.checkpoint,
            input_required=["action"],
        )
        self._methods["artifact"] = ServiceMethod(
            name="artifact",
            description="artifact 管理 (register/list/validate/pane/layout)",
            func=self.artifact,
            input_required=["action"],
        )
    ...
    def config(
        ...
        ConfigStack, ConfigLayer, ConfigEntry, stack_to_dict, merge_layers, PermissionRegistry = (
            _load_config_stack()
        )
        ...
        if action == "get":
            ...
            return {
                ...
                "layer": entry.layer.value if entry else None,
            }
        if action == "set":
    ...
    def artifact(
        ...
        if action == "validate":
            return validate(
                id=id or "", name=name or "", type=type or "agent", description=description or ""
            )
        if action == "add_pane":
            return add_pane(
                pane_id=pane_id or "",
