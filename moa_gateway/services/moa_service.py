...
def _load_moa_engine():
    from ..capability.moa_engine import Proposer, Aggregator, run_moa, validate_moa
...
class MoAService(ServiceBase):
    ...
    async def run_three_layer(self, query, proposers, aggregators,
                              temperature=0.6, max_total_tokens=0):
        ...
        result = await run_three_layer_moa(
            query, proposers=ps, aggregators=aggs,
            temperature=temperature, max_total_tokens=max_total_tokens,
        )
    ...
    async def run_engine(self, query, proposers, aggregator,
                         validate_only=False, temperature=0.6):
    ...
    def cross_iter(self, iters, action):
        ...
        raise ValueError(f"unknown action: {action}, expected one of convergence/best_of_each/adoption/step5")
