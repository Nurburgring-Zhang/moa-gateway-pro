...
def _load_channels():
    from ..capability.channels import (
        ChannelChain, classify_error, ChannelType,
    )
...
class RoutingService(ServiceBase):
    ...
    def _register_methods(self):
        self._methods["route"] = ServiceMethod(
            name="route", description="根据 query 复杂度路由到合适模型",
            func=self.route,
            input_required=["query"], input_optional=["model_hint", "strategy"],
        )
        self._methods["chain_info"] = ServiceMethod(
            name="chain_info", description="获取通道 chain 拓扑信息",
            func=self.chain_info,
        )
        self._methods["execute_chain"] = ServiceMethod(
            name="execute_chain", description="沿 chain 顺序执行",
            func=self.execute_chain,
            input_required=["query", "enabled"],
        )
        self._methods["classify_error"] = ServiceMethod(
            name="classify_error", description="错误分类(rate_limit / auth / quota / transient)",
        ...
        self._methods["reference_route"] = ServiceMethod(
            name="reference_route", description="Reference Router: 影子模式/对比",
    ...
    def chain_info(self):
        ...
        return {
            ...
            "topology": [
                {"name": c.name, "type": str(c.channel_type)}
    ...
    def reference_route(self, query, main_model, ref_model, strategy="shadow",
                        max_latency_ms=5000, cost_ratio_cap=2.0):
        ReferenceRouter = _load_reference_router()
        rr = ReferenceRouter(
            main_model=main_model, ref_model=ref_model,
            strategy=strategy, max_latency_ms=max_latency_ms, cost_ratio_cap=cost_ratio_cap,
        )
        return {"strategy": strategy, "main": main_model, "ref": ref_model,
