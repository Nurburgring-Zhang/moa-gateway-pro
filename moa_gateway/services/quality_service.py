...
def _load_elo():
    from ..capability.elo_ranking import record_match, get_ranked, submit_workers
...
def _load_meta_prompt():
    from ..capability.meta_prompt import get_stages, clash, fuse
...
class QualityService(ServiceBase):
    ...
    def _register_methods(self):
        self._methods["score_flask"] = ServiceMethod(
            name="score_flask", description="FLASK 多维度评分 (Truthfulness/Logic/etc)",
            func=self.score_flask,
            input_required=["query", "response"], input_optional=["tasks"],
        )
        self._methods["rank_elo"] = ServiceMethod(
            name="rank_elo", description="ELO 排名 (record / ranked / submit workers)",
            func=self.rank_elo,
            input_required=["action"],
            input_optional=["model_ids", "matches", "k_factor", "workers", "strategy"],
        )
        self._methods["gate_l0"] = ServiceMethod(
            name="gate_l0", description="L0 安全门禁: 检测有害 query",
            func=self.gate_l0,
            input_required=["query"],
        )
        self._methods["score_panel"] = ServiceMethod(
            name="score_panel", description="多维度评分面板",
            func=self.score_panel,
            input_required=["query", "answer"], input_optional=["criteria"],
        )
        self._methods["brainstorm"] = ServiceMethod(
            name="brainstorm", description="头脑风暴 (ideas / decide)",
            func=self.brainstorm,
            input_required=["topic", "action"],
        )
        self._methods["plan_act"] = ServiceMethod(
            name="plan_act", description="plan+act 编排",
            func=self.plan_act, is_async=True,
            input_required=["query"],
        )
        self._methods["meta_prompt"] = ServiceMethod(
            name="meta_prompt", description="meta prompt 模板 (stages / clash / fuse)",
    ...
    def rank_elo(self, action, model_ids=None, matches=None, k_factor=4.0,
                 workers=None, strategy="lottery"):
