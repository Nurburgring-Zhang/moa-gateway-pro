    ...
    for p in prompts:
        ...
        try:
            ...
            if use_flask and res.final_content:
                try:
                    flask = await moa.flask_score(p["text"], res.final_content,
...
async def run_pareto(prompts: List[str],
    ...
    bench_prompts = [
        {"id": f"p{i}", "category": "custom", "text": p}
    ...
    for preset_name in presets:
        try:
            ...
            avg_score = (
                sum(i.get("flask_avg", 0) or 0 for i in r["items"]) /
                max(1, sum(1 for i in r["items"] if i.get("flask_avg")))
            )
    ...
    frontier = []
    sorted_by_cost = sorted([pt for pt in points if "error" not in pt],
    ...
    if points:
        max_score = max((pt["avg_score"] for pt in points
                         if "error" not in pt), default=0)
        threshold = max_score * 0.8
        candidates = [pt for pt in points
                      if "error" not in pt and pt["avg_score"] >= threshold]
        if candidates:
