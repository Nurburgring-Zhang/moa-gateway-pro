...
class IntelligentRouter:
    ...
    QUESTION_PATTERNS = {
        # 英文
        "why": 20, "how": 25, "what": 10, "which": 10,
        "compare": 30, "analyze": 35, "design": 40, "implement": 35,
        "debug": 30, "architecture": 45, "strategy": 40,
        "optimize": 35, "review": 30, "evaluate": 30, "refactor": 30,
        "explain": 15, "summarize": 12, "translate": 8, "list": 8,
        # 中文
        "为什么": 20, "怎么": 22, "如何": 22, "什么": 8, "哪个": 10,
        "比较": 28, "对比": 28, "分析": 35, "设计": 38, "实现": 32,
        "调试": 28, "架构": 45, "策略": 38, "优化": 32, "评估": 28,
        "重构": 28, "解释": 12, "总结": 10, "翻译": 6, "列举": 6,
        "权衡": 30, "取舍": 30, "规划": 25, "排查": 28, "审查": 28,
        "评审": 28, "选型": 30, "落地": 22, "改造": 25, "迁移": 25,
    }

    TECH_KEYWORDS = [
        # 英文
        "architecture", "system design", "distributed", "concurrency",
        "security", "authentication", "database", "scalability",
        "kubernetes", "microservice", "devops", "infrastructure",
        "machine learning", "neural network", "algorithm",
        "compiler", "kernel", "protocol", "cryptography", "blockchain",
        "quantum", "distributed system", "consensus", "raft", "paxos",
        "kafka", "redis", "postgres", "mongodb", "graphql", "rest",
        "tla+", "formal verification", "proof", "theorem", "complexity",
        # 中文
        "分布式", "高并发", "微服务", "数据库", "架构", "安全",
        "认证", "授权", "可扩展", "可伸缩", "区块链", "加密",
        "算法", "编译器", "内核", "协议", "消息队列", "缓存",
        "事务", "索引", "锁", "并发", "性能", "延迟", "吞吐",
        "sql", "nosql", "图数据库", "时序", "日志", "监控",
        "rpc", "http", "tcp", "udp", "tls", "ssl", "jwt", "oauth",
        "容器", "服务网格", "网关", "负载均衡", "熔断", "限流",
    ]

    # 多步推理特征
    MULTI_STEP_INDICATORS = [
        "step by step", "first", "then", "finally", "and then",
        "compare and contrast", "tradeoff", "pros and cons",
        "1.", "2.", "3.",
        # 中文
    ...
    def estimate_cost(self, query: str, ep: ModelEndpoint,
                      estimated_output_tokens: int = 600) -> float:
        """估算调用成本(USD)"""
    ...
    def route(self, query: str, context: Optional[List[Dict]] = None,
        ...
        if not primary:
            return RoutingDecision(
                complexity=complexity, tier=tier, primary=None,
                fallback_chain=[],
                estimated_cost=0.0,
        ...
        return RoutingDecision(
            complexity=complexity, tier=tier, primary=primary,
    ...
    def route_for_moa(self, query: str, context: Optional[List[Dict]] = None,
        ...
        if len(refs) < reference_count:
            ...
            for delta in range(1, 5):
                ...
                if up == tier:  # 已经到顶
                    break
                extras = self.pool.select_many(up, reference_count - len(refs),
