...
class MoAResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "references": [
                {
                    "model_id": r.model_id, "role": r.role, "success": r.success,
            ...
            "critics": [
                {
                    "model_id": c.model_id, "success": c.success,
            ...
            "chain_steps": [
                {
                    "step": s.step, "strategy": s.strategy, "preset": s.preset,
                    "latency_ms": round(s.latency_ms, 1), "cost": round(s.cost, 6),
                    "preview": (s.output or "")[:200] + ("..." if len(s.output or "") > 200 else ""),
...
class MoAOrchestrator:
    ...
    async def execute(self, query: str,
        ...
        start = time.time()
        result = MoAResult(
            request_id=request_id, query=query, preset="", strategy=""
        )
        ...
        try:
            if strat == "single" or (preset_cfg.reference_models == [] and ref_n <= 1):
                await self._run_single(result, messages, tools, agg_id,
                                        ref_temp, max_tok, start)
            elif strat == "compose":
                await self._run_compose(result, messages, tools,
                                          preset_cfg, ref_temp, agg_temp, max_tok, start)
            elif strat == "judge":
                await self._run_judge(result, messages, tools,
                                       preset_cfg, agg_id, ref_temp, max_tok, start)
            elif strat == "chain":
                await self._run_chain(result, messages, tools,
                                       preset_cfg, ref_temp, agg_temp, max_tok, start)
            elif strat == "pipeline":
                await self._run_pipeline(result, messages, tools,
                                          preset_cfg, ref_temp, max_tok, start)
            elif strat == "layered":
                # Together AI MoA 真多层 L1→L2→L3
                await self._run_layered(result, messages, tools,
                                         preset_cfg, agg_id, ref_temp, agg_temp,
                                         max_tok, start)
            elif strat == "single_proposer":
                # 同模型高温多次采样(论文:Single-Proposer setting)
                await self._run_single_proposer(result, messages, tools,
                                                  preset_cfg, agg_id, ref_temp,
                                                  max_tok, start)
            elif strat == "ranker":
                # LLM Ranker baseline(论文 Figure 4:让 aggregator 选而不是生成)
                await self._run_ranker(result, messages, tools,
                                         preset_cfg, agg_id, ref_temp, max_tok, start)
            else:  # parallel
                await self._run_parallel(result, messages, tools,
                                           preset_cfg, ref_n, agg_id,
        ...
        logger.info("MoA[%s] strategy=%s preset=%s done in %.0fms cost=$%.4f",
                    request_id, result.strategy, result.preset,
                    result.total_latency_ms, result.total_cost)
        return result
    ...
    async def _run_single(self, result: MoAResult, messages, tools,
        ...
        resp = await self._call_with_fallback(ep, messages, tools,
        ...
        result.references = [ReferenceResult(
            model_id=ep.id, content=resp["content"], success=True,
            prompt_tokens=resp.get("prompt_tokens", 0),
            completion_tokens=resp.get("completion_tokens", 0),
            cost=resp["cost"], latency_ms=resp.get("latency_ms", 0)
        )]
    ...
    async def _run_parallel(self, result: MoAResult, messages, tools,
        ...
        ref_results = await self._run_references(ref_endpoints, messages,
                                                  ref_temp, max_tokens)
        result.references = ref_results
        ok_count = sum(1 for r in ref_results if r.success)
        if ok_count == 0:
            ref_results = [ReferenceResult(
                model_id="system",
        ...
        agg_resp = await self._call_with_fallback(aggregator_ep, agg_messages, tools,
                                                  agg_temp, max_tokens)
        ...
        for r in range(critic_rounds):
            critic = await self._run_critic(current_content, ref_results,
                                             aggregator_ep, agg_temp, max_tokens)
    ...
    # ========== compose(多角度分工) ==========
    async def _run_compose(self, result: MoAResult, messages, tools,
        ...
        if explicit_refs and any(r.role for r in explicit_refs):
            ...
            if needed:
                fallback_refs = self.pool.select_many(
                    ModelTier(preset_cfg.tier), needed,
            ...
            ref_endpoints = self.pool.select_many(
                ModelTier(preset_cfg.tier), preset_cfg.reference_count,
                prefer_diversity=True
            )
        ...
        # aggregator
        aggregator = (self.pool.endpoints.get(preset_cfg.aggregator)
                       if preset_cfg.aggregator else None)
        if not aggregator:
        ...
        for ep in ref_endpoints:
            ...
            tasks.append(self._call_one_ref_with_messages(ep, role_messages,
        ...
        agg_messages = self._build_compose_aggregator_messages(
            messages, ref_results, roles_used)
        agg_resp = await self._call_with_fallback(aggregator, agg_messages, tools,
                                                  agg_temp, max_tokens)
        result.aggregated_content = agg_resp["content"]
    ...
    def _build_compose_aggregator_messages(self, original, ref_results, roles_used):
        ...
        ref_text = "\n\n".join([
            ...
            if r.success else f"【{r.role} 视角 - 模型 {r.model_id}】失败: {r.error}"
    ...
    async def _run_judge(self, result: MoAResult, messages, tools,
        ...
        # 第一轮:正常回答
        resp1 = await self._call_with_fallback(ep, messages, tools,
                                                temperature, max_tokens)
        ...
        for r in range(preset_cfg.critic_rounds):
            ...
            try:
                resp2 = await self._call_with_fallback(ep, critique_msgs, tools,
                ...
                current = new_content
                result.iterations = r + 2
                result.critics.append(CriticResult(
                    model_id=ep.id, success=True, refined_content=new_content,
                    latency_ms=resp2.get("latency_ms", 0), cost=resp2["cost"]
                ))
            except Exception as e:
    ...
    # ========== chain(链式 MoA) ==========
    async def _run_chain(self, result: MoAResult, messages, tools,
                          preset_cfg: MoAPresetConfig,
                          ref_temp: float, agg_temp: float,
                          max_tokens: int, start: float):
        ...
        for idx, stage in enumerate(steps):
            step_result = ChainStepResult(step=idx + 1, strategy="chain",
                                          preset=stage.name, output="", cost=0)
            # 加载此 step 的角色 prompt(可被用户自定义)
            role_prompt = get_prompt(f"chain_{stage.name}")
            sub_preset = MoAPresetConfig(
                enabled=True, strategy="parallel",
                reference_count=3,
                tier=stage.tier,
            ...
            sub_orch = MoAOrchestrator(model_pool=self.pool,
                                        router=self.router)
            ...
            try:
                ...
                sub_res = await sub_orch.execute(
                    query=sub_q, context=full_context, tools=tools,
                    preset="__chain_step__", critic_rounds=0
                )
        ...
        result.aggregated_content = (result.chain_steps[-1].output
                                     if result.chain_steps else "")
        result.final_content = result.aggregated_content
        result.aggregator_model = "chain"

    # ========== pipeline ==========
    async def _run_pipeline(self, result: MoAResult, messages, tools,
                              preset_cfg: MoAPresetConfig,
                              temperature: float, max_tokens: int, start: float):
        ...
        if not stages:
            from .config import MoAStageConfig
            stages = [MoAStageConfig(name="planner", tier="premium"),
        ...
        for stage in stages:
            ...
            if stage.name == "planner":
                stage_messages = list(messages) + [
                    {"role": "user", "content":
                     "请把用户需求细化成可执行的任务规格(spec)。"
                     "包含:目标、关键步骤、输出格式、约束、验收标准。"}
                ]
            elif stage.name == "evaluator":
                stage_messages = list(current_messages) + [
                    {"role": "user", "content":
                     "请评审上一阶段的输出,若有问题请给出修订版,否则输出'PASS'。"}
                ]
            resp = await self._call_with_fallback(ep, stage_messages, tools,
                                                  temperature, max_tokens)
    ...
    async def _run_references(self, refs, messages, temperature, max_tokens):
        ...
        tasks = [self._call_one_ref_with_messages(ep, messages, temperature,
                                                  max_tokens, role="")
                 for ep in refs]
    ...
    async def _call_one_ref_with_messages(self, ep, messages, temperature, max_tokens,
                                           role: str = "") -> ReferenceResult:
        """调用单个参考模型 + 自动超时 + 异常处理(返回 ReferenceResult)"""
        start = time.time()
        try:
            resp = await asyncio.wait_for(
                self.pool.call(ep.id, messages, temperature=temperature,
                               max_tokens=max_tokens, tools=None, max_retries=2),
                timeout=self.settings.moa.reference_timeout
            )
            return ReferenceResult(
                model_id=ep.id, content=resp.content, success=True, role=role,
            ...
            return ReferenceResult(model_id=ep.id, content="", success=False,
                                    role=role,
    ...
    def _build_aggregator_messages(self, original, ref_results):
        ...
        ref_text = "\n\n".join([
            ...
            if r.success else f"【参考 {i+1} — 模型: {r.model_id}】(调用失败: {r.error})"
            for i, r in enumerate(ref_results)
        ])
        messages.append({"role": "user", "content": f"# 原始问题\n{user_q}\n\n# 多模型参考回答\n{ref_text}\n\n# 你的任务\n请综合以上参考,给出最终答案。在最终答案开头用一句话说明你的综合判断依据。"})
        return messages
    ...
    async def _call_with_fallback(self, ep, messages, tools, temperature, max_tokens):
        ...
        for attempt, cur in enumerate(chain):
            ...
            try:
                resp = await asyncio.wait_for(
                    self.pool.call(cur.id, messages, temperature=temperature,
                                   max_tokens=max_tokens, tools=tools, max_retries=1),
                    timeout=self.settings.moa.aggregator_timeout
                )
                return {
                    "content": resp.content, "cost": resp.cost,
    ...
    async def _run_critic(self, current_content, ref_results, aggregator_ep,
                           temperature, max_tokens):
        start = time.time()
        critic_pool = [e for e in self.pool.endpoints.values()
                       if e.is_available and e.id != aggregator_ep.id]
        if not critic_pool:
        ...
        try:
            resp = await asyncio.wait_for(
                self.pool.call(critic_ep.id, critic_messages, temperature=0.3,
                               max_tokens=1500, max_retries=1),
                timeout=self.settings.moa.aggregator_timeout
            )
            content = resp.content.strip()
            issues, suggestions = self._parse_critic_output(content)
            return CriticResult(model_id=critic_ep.id, issues=issues,
                                 suggestions=suggestions, refined_content=content,
                                 success=True, latency_ms=(time.time() - start) * 1000,
                                 cost=resp.cost)
        except Exception as e:
            return CriticResult(model_id=critic_ep.id, success=False,
    ...
    async def evaluate(self, query: str, candidates: List[str],
        ...
        for cand_id in candidates:
            ...
            # 直接调出答案
            try:
                resp = await self.pool.call(cand_id,
                                              [{"role": "user", "content": query}],
                                              temperature=temperature, max_tokens=2048)
                results.append({"model": cand_id, "answer": resp.content,
...
```""" % (reference_answer or "(无)",
    ...
    async def _run_layered(self, result: MoAResult, messages, tools,
        ...
        # L1: 选参考模型
        l1_eps, _ = self._resolve_models(preset_cfg, preset_cfg.reference_count,
                                          aggregator_id)
        if not l1_eps:
        ...
        for layer_idx in range(1, layer_count + 1):
            ...
            if is_final and aggregator_id and self.pool.endpoints.get(aggregator_id):
                ...
                current_eps.sort(key=lambda e: -tier_order.get(
            ...
            for ep in current_eps:
                if layer_idx == 1:
                    ...
                    user_q = next((m.get("content", "") for m in messages
                                   if m.get("role") == "user"), "")
                    layer_msgs = [
                ...
                tasks.append(self._call_one_ref_with_messages(
                    ep, layer_msgs,
            ...
            for ep, r in zip(current_eps, responses):
                if isinstance(r, Exception):
                    layer_outputs.append(ReferenceResult(
                        ...
                        error=str(r), role=f"L{layer_idx}", tier=str(ep.tier.value)))
                else:
        ...
        logger.info("Layered MoA done: %d layers, %d total refs, $%.4f",
                     layer_count, len(prev_results), result.total_cost)
    ...
    async def _run_single_proposer(self, result: MoAResult, messages, tools,
        ...
        for i, r in enumerate(responses):
            if isinstance(r, Exception):
                ref_results.append(ReferenceResult(
                    model_id=proposer_ep.id, content="", success=False,
                    error=str(r), role=f"sample{i+1}"))
            else:
                ref_results.append(ReferenceResult(
                    model_id=proposer_ep.id, content=r.content,
                    success=True,
                    prompt_tokens=getattr(r, "prompt_tokens", 0),
                    completion_tokens=getattr(r, "completion_tokens", 0),
                    cost=getattr(r, "cost", 0.0),
        ...
        agg_ep = (self.pool.endpoints.get(aggregator_id)
                   if aggregator_id else None)
        if not agg_ep:
        ...
        agg_messages = self._build_aggregator_messages(messages, ref_results)
        agg_resp = await self._call_with_fallback(agg_ep, agg_messages, tools,
                                                    preset_cfg.aggregator_temperature,
                                                    max_tokens)
        result.final_content = agg_resp["content"]
        result.aggregated_content = agg_resp["content"]
        result.total_cost += agg_resp["cost"]
    ...
    async def _run_ranker(self, result: MoAResult, messages, tools,
        ...
        # 并行调用所有 reference
        ref_eps, _ = self._resolve_models(preset_cfg, preset_cfg.reference_count,
                                            aggregator_id)
        if not ref_eps:
            raise RuntimeError("no available model for ranker")
        tasks = [self._call_one_ref_with_messages(
        ...
        for ep, r in zip(ref_eps, responses):
            if isinstance(r, Exception):
                ref_results.append(ReferenceResult(
                    model_id=ep.id, content="", success=False,
                    error=str(r), role="ranker_candidate"))
            else:
                ref_results.append(ReferenceResult(
                    model_id=ep.id, content=r.content,
                    success=True,
                    prompt_tokens=getattr(r, "prompt_tokens", 0),
                    completion_tokens=getattr(r, "completion_tokens", 0),
                    cost=getattr(r, "cost", 0.0),
        ...
        cands_text = "\n\n".join(
            ...
            for i, r in enumerate(ref_results) if r.success)
