...
def repair_venv() -> bool:
    ...
    rc, out, err = _run(
        [sys.executable, "-m", "venv", "--upgrade-deps", str(VENV_DIR)], timeout=180
    )
    if rc != 0:
...
def _pip_install(
    specs: list[str], mirrors: list[tuple[str, str]] = PIP_MIRRORS, retries_per_mirror: int = 2
) -> bool:
    ...
    for url, name in mirrors:
        for attempt in range(retries_per_mirror):
            ...
            _log(
                f"[pip] 试源 {name} ({url}) 第 {attempt + 1}/{retries_per_mirror} 次: {' '.join(specs)}"
            )
