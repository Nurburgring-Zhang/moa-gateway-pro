...
if not _INJECTION_RAW:
    _INJECTION_RAW = [
        ...
        (
            "MARKDOWN_HEADER_INJECT",
            r"(?im)^#{1,6}\s*(?:system|assistant|user|instruction|prompt)\s*[:：]",
        ),
...
_WRAP_HEADER_FMT = (
    '<untrusted_tool_output source="{source}" trust="{trust}" length="{length}"{truncated_attr}>\n'
)
...
_TRUNCATED_INSIDE_RE = re.compile(r'<truncated\s+total_length="(?P<n>\d+)"\s*/>')
