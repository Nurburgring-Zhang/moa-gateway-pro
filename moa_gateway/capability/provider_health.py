...
def compute_score(metrics: HealthMetrics) -> HealthScore:
    ...
    if metrics.avg_latency_ms > 10000:
        ...
        reasons.append(f"avg_latency={metrics.avg_latency_ms:.0f}ms > 10000 -> -{_PEN_LATENCY_OVER_10S}")
    elif metrics.avg_latency_ms > 5000:
        score -= _PEN_LATENCY_OVER_5S
        reasons.append(f"avg_latency={metrics.avg_latency_ms:.0f}ms > 5000 -> -{_PEN_LATENCY_OVER_5S}")
    ...
    if metrics.avg_latency_ms > 0 and metrics.avg_latency_ms < 1000:
        ...
        reasons.append(f"avg_latency={metrics.avg_latency_ms:.0f}ms < 1000 -> +{_BONUS_LOW_LATENCY}")
