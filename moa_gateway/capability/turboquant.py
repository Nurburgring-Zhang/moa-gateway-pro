...
class TurboQuantConfig:
    ...
    def __post_init__(self) -> None:
        if self.hard_cap <= 0:
            raise ValueError(f"hard_cap must be > 0, got {self.hard_cap}")
        if self.preserve < 0:
            raise ValueError(f"preserve must be >= 0, got {self.preserve}")
        if self.preserve > self.hard_cap:
            raise ValueError(f"preserve ({self.preserve}) must be <= hard_cap ({self.hard_cap})")
        if not isinstance(self.level, QuantLevel):
            raise TypeError(f"level must be QuantLevel, got {type(self.level).__name__}")
...
def compress_message(msg: Message, level: QuantLevel) -> Message:
    ...
    if not isinstance(level, QuantLevel):
        raise TypeError(f"level must be QuantLevel, got {type(level).__name__}")
...
def should_compress(
    ...
    if not isinstance(config, TurboQuantConfig):
        raise TypeError(f"config must be TurboQuantConfig, got {type(config).__name__}")
    if not messages:
        return False
