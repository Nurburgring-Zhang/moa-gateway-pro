...
_BATTLE_PATTERNS = [
    ...
    (
        re.compile(
            r"\bA\s+is\s+(?:better|superior|more\s+(?:helpful|accurate|relevant))\b", re.IGNORECASE
        ),
        "A",
    ),
    (
        re.compile(
            r"\bB\s+is\s+(?:better|superior|more\s+(?:helpful|accurate|relevant))\b", re.IGNORECASE
        ),
        "B",
    ),
