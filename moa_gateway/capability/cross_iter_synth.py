...
def convergence_mode(iters: list[IterationSnapshot]) -> SynthesisResult:
    ...
    if len(iters) == 1:
        ...
        out_kws = sorted(only)
        output = (
            "Single iteration; convergent keywords: " + ", ".join(out_kws)
            if out_kws
            else "Single iteration; no keywords"
        )
        return SynthesisResult(
            mode=SynthesisMode.CONVERGENCE,
            output=output,
...
def _self_improve_suggestions(synth: SynthesisResult) -> list[str]:
    ...
    if not synth.sources:
        ...
        suggestions.append(
            "Few iterations available; recommend at least 3 rounds for stable patterns."
        )
    suggestions.append(
        "Cross-compare top proposals; merge complementary parts where overlap is high."
    )
