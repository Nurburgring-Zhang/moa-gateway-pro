...
_RAW_PATTERNS: tuple[tuple[int, str, str, RiskLevel], ...] = (
    # ---- 1) SQL injection ----------------------------------------------------
    (1, "SQL_UNION_SELECT", r"(?i)\bunion\b\s+(?:all\s+)?\bselect\b", RiskLevel.HIGH),
    (
        1,
        "SQL_DROP_TABLE",
    ...
    (1, "SQL_DELETE_WHERE", r"(?i)\bdelete\s+from\s+\w+\s*(?:where)?", RiskLevel.HIGH),
    (1, "SQL_UPDATE_SET", r"(?i)\bupdate\s+\w+\s+set\s+\w+\s*=", RiskLevel.HIGH),
    (1, "SQL_EXEC_SP", r"(?i)\bexec(?:ute)?\s+(?:sp_|xp_)\w+", RiskLevel.BLOCKED),
    (1, "SQL_TAUTOLOGY", r"(?i)'\s*or\s+['\d]+\s*=\s*['\d]+", RiskLevel.MEDIUM),
    (1, "SQL_COMMENT_TRAIL", r"(?i)--\s*$.*|/\*.*?\*/", RiskLevel.LOW),
    # ---- 2) Shell command ----------------------------------------------------
    (2, "SHELL_RM_RF", r"\brm\s+(?:-[a-zA-Z]*\s+)*-?[a-zA-Z]*[rR][fF]\b", RiskLevel.BLOCKED),
    (2, "SHELL_CURL_PIPE_SH", r"\bcurl\b[^\n|]*\|\s*(?:ba)?sh\b", RiskLevel.BLOCKED),
    (2, "SHELL_WGET_PIPE_SH", r"\bwget\b[^\n|]*\|\s*(?:ba)?sh\b", RiskLevel.BLOCKED),
    (2, "SHELL_EVAL", r"\beval\s*[\(\s]", RiskLevel.HIGH),
    (2, "SHELL_DD", r"\bdd\s+if=\S+\s+of=\S+", RiskLevel.HIGH),
    (2, "SHELL_FORK_BOMB", r":\(\)\s*\{\s*:\|:&\s*\};\s*:", RiskLevel.BLOCKED),
    (
        2,
        "SHELL_NC_REVERSE",
    ...
    # ---- 3) Path traversal ---------------------------------------------------
    (3, "PATH_DOTDOT_SLASH", r"(?:\.\./){2,}", RiskLevel.HIGH),
    (3, "PATH_DOTDOT_BACKSLASH", r"(?:\.\.\\){2,}", RiskLevel.HIGH),
    (3, "PATH_ABS_ETC_PASSWD", r"/etc/(?:passwd|shadow|hosts|sudoers)\b", RiskLevel.BLOCKED),
    (3, "PATH_ABS_WINDOWS_SYSTEM", r"[A-Za-z]:\\Windows\\System32\\", RiskLevel.BLOCKED),
    (3, "PATH_PROC_SELF", r"/proc/self/(?:environ|cmdline|maps)\b", RiskLevel.HIGH),
    (3, "PATH_DEV_NULL", r"/dev/(?:null|zero|random)\b", RiskLevel.LOW),
    # ---- 4) Code injection ---------------------------------------------------
    (4, "PY_EXEC_CALL", r"\bexec\s*\(", RiskLevel.HIGH),
    (4, "PY_EVAL_CALL", r"\beval\s*\(", RiskLevel.HIGH),
    (4, "PY_DUNDER_IMPORT", r"__import__\s*\(", RiskLevel.HIGH),
    (4, "PY_SUBPROCESS", r"\bsubprocess\.(?:Popen|run|call|check_output)\s*\(", RiskLevel.HIGH),
    (4, "PY_OS_SYSTEM", r"\bos\.system\s*\(", RiskLevel.HIGH),
    (4, "JS_FUNCTION_CONSTRUCTOR", r"\bnew\s+Function\s*\(", RiskLevel.HIGH),
    (4, "PY_PICKLE_LOADS", r"\bpickle\.loads?\s*\(", RiskLevel.MEDIUM),
    # ---- 5) Prompt injection -------------------------------------------------
    (5, "PI_IGNORE_PREVIOUS", r"(?i)ignore\s+(?:all\s+)?(?:the\s+)?previous", RiskLevel.HIGH),
    (
        5,
        "PI_DISREGARD",
        r"(?i)\bdisregard\s+(?:the\s+)?(?:above|prior|previous|instructions)",
        RiskLevel.HIGH,
    ),
    (5, "PI_SYSTEM_TAG", r"(?im)^\s*system\s*:\s*", RiskLevel.MEDIUM),
    (5, "PI_ASSISTANT_TAG", r"(?im)^\s*assistant\s*:\s*", RiskLevel.LOW),
    (5, "PI_JAILBREAK", r"(?i)\bDAN\s+mode\b|\bjailbreak\b", RiskLevel.HIGH),
    (5, "PI_REVEAL_PROMPT", r"(?i)reveal\s+(?:the\s+)?system\s+prompt", RiskLevel.MEDIUM),
    # ---- 6) URL fetch (SSRF) -------------------------------------------------
    (
        6,
        "URL_LOOPBACK",
    ...
    (6, "URL_AWS_METADATA", r"\bhttps?://169\.254\.169\.254(?:/\S*)?", RiskLevel.BLOCKED),
    (6, "URL_FILE_SCHEME", r"\bfile:///\S+", RiskLevel.BLOCKED),
    (6, "URL_GOPHER_SCHEME", r"\bgopher://\S+", RiskLevel.BLOCKED),
    (6, "URL_DICT_SCHEME", r"\bdict://\S+", RiskLevel.HIGH),
    ...
    (7, "WRITE_PASSWD", r"/etc/(?:passwd|shadow|sudoers|hosts)\b", RiskLevel.BLOCKED),
    (
        7,
        "WRITE_WINDOWS_SYSTEM32",
        r"[A-Za-z]:\\Windows\\(?:System32|SysWOW64)\\",
        RiskLevel.BLOCKED,
    ),
    (7, "WRITE_SSH_AUTHORIZED", r"\.ssh/authorized_keys\b", RiskLevel.BLOCKED),
    (7, "WRITE_BASHRC", r"(?:\.bashrc|\.bash_profile|\.zshrc|\.profile)\b", RiskLevel.HIGH),
    (7, "WRITE_CRON", r"/etc/cron\.\S+|/var/spool/cron/\S+", RiskLevel.HIGH),
    ...
    (8, "NET_BIG_BASE64", r"[A-Za-z0-9+/]{2048,}={0,2}", RiskLevel.HIGH),
    (
        8,
        "NET_POST_SENSITIVE_KEY",
        r"(?i)POST[^\n]{0,80}(?:api[_-]?key|token|password|secret)\s*[=:]",
        RiskLevel.HIGH,
    ),
    (
        8,
        "NET_HTTPS_TO_SUSPICIOUS",
    ...
    (8, "NET_AWS_KEY_PAIR", r"(?i)aws_access_key_id\s*=\s*AKIA[0-9A-Z]{16}", RiskLevel.BLOCKED),
    # ---- 9) Privilege escalation --------------------------------------------
    (9, "PRIV_SUDO", r"\bsudo\s+(?!-?[lnVHEh])\S+", RiskLevel.HIGH),
    (9, "PRIV_SUDO_SU", r"\bsudo\s+su\b", RiskLevel.BLOCKED),
    (9, "PRIV_CHMOD_777", r"\bchmod\s+(-[a-zA-Z]+\s+)?777\b", RiskLevel.HIGH),
    (9, "PRIV_CHOWN_ROOT", r"\bchown\s+(-[a-zA-Z]+\s+)?root(?::root)?\b", RiskLevel.BLOCKED),
    (9, "PRIV_SETUID_BIT", r"\bchmod\s+[0-7]*[4-7][0-7]{2}\b", RiskLevel.HIGH),
    (9, "PRIV_SU_PLAIN", r"(?<![\w/])su\s+-\s*\w+", RiskLevel.MEDIUM),
)
...
def _build(entries: Sequence[tuple[int, str, str, RiskLevel]]) -> tuple[_Compiled, ...]:
    ...
    for cat, pid, raw, risk in entries:
        try:
            out.append(_Compiled(category=cat, pattern_id=pid, regex=re.compile(raw), risk=risk))
...
class ToolScreener:
    ...
    def _compile(self, custom: dict[Any, Any] | None) -> tuple[_Compiled, ...]:
        ...
        for cat, pid, raw, risk in entries:
            try:
                by_cat[cat].append(
                    _Compiled(category=cat, pattern_id=pid, regex=re.compile(raw), risk=risk)
                )
...
def _cli(argv: list[str] | None = None) -> int:  # pragma: no cover
    ...
    parser.add_argument("--args", default="{}", help="JSON-encoded arguments object")
