...
def get_stage_prompts(query: str, roles: Optional[List[str]] = None) -> List[MetaPrompt]:
    ...
    s1_user_tpl = STAGE1_TEMPLATES.get(
        primary_role, STAGE1_TEMPLATES["answerer"]
    )
    s1 = MetaPrompt(
...
async def _call_provider(provider: Any, system: str, user: str,
    ...
    for i, prompt in enumerate(prompts):
        ...
        try:
            output = await _call_provider(
                provider, prompt.system_prompt, user_content, model=model
            )
...
def three_jumps(initial_output: str) -> List[str]:
    ...
    for i, p in enumerate(perspectives):
        ...
        debate_pairs.append(
            f"Critic challenges {p[:50]}... with {opponent[:50]}..."
        )
...
# 决策权重用的真实关键词
DECISION_KEYWORDS = {
    "high": ["must", "should", "required", "necessary", "critical", "essential",
             "必须", "应该", "必要", "关键", "核心", "重要"],
    "low": ["maybe", "perhaps", "might", "could", "possibly", "optionally",
            "可能", "也许", "或许", "大概", "似乎"],
}
...
def fuse_decision(options: List[str], context: str = "") -> str:
    ...
    for idx, opt in enumerate(valid):
        ...
        # 2. 关键词密度
        high_kw = sum(1 for kw in DECISION_KEYWORDS["high"]
                      if kw.lower() in opt_lc)
        low_kw = sum(1 for kw in DECISION_KEYWORDS["low"]
                     if kw.lower() in opt_lc)
