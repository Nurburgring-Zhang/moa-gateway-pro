...
def cosine_similarity(a: list[float], b: list[float]) -> float:
    ...
    """
    if len(a) != len(b):
        raise ValueError(f"cosine_similarity: length mismatch a={len(a)} vs b={len(b)}")
    if not a:
        return 0.0
    dot = 0.0
    norm_a_sq = 0.0
...
def euclidean_distance(a: list[float], b: list[float]) -> float:
    ...
    """
    if len(a) != len(b):
        raise ValueError(f"euclidean_distance: length mismatch a={len(a)} vs b={len(b)}")
    s = 0.0
    for x, y in zip(a, b, strict=False):
        d = x - y
        s += d * d
...
class EmbeddingIndex:
    ...
    def add_batch(self, items: list[tuple[str, list[float]]]) -> list[int]:
    ...
    def search(self, query_vec: list[float], top_k: int = 5) -> list[tuple[int, float, str]]:
        ...
        if len(query_vec) != self.dim:
            raise ValueError(f"query_vec dim {len(query_vec)} != index dim {self.dim}")
        if top_k < 1:
            return []
