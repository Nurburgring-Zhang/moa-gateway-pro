...
class MultiKeyTokenBucket:
    ...
    def __init__(
        ...
        if not isinstance(default_capacity, (int, float)) or isinstance(default_capacity, bool):
            raise TypeError(
                f"default_capacity must be numeric, got {type(default_capacity).__name__}"
            )
        if default_capacity < 0:
            raise ValueError(f"default_capacity must be >= 0, got {default_capacity}")
        if not isinstance(default_refill_rate, (int, float)) or isinstance(
            default_refill_rate, bool
        ):
            raise TypeError(
                f"default_refill_rate must be numeric, got {type(default_refill_rate).__name__}"
            )
