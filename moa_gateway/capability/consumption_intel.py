...
class RequestContext:
    ...
    def __post_init__(self) -> None:
        if not self.request_id or not isinstance(self.request_id, str):
            raise ValueError(
                f"request_id must be non-empty str, got {self.request_id!r}"
            )
        if self.priority not in ("low", "normal", "high"):
            raise ValueError(
                f"priority must be low/normal/high, got {self.priority!r}"
            )
        if self.max_cost_per_1k is not None and self.max_cost_per_1k < 0:
            raise ValueError(
                f"max_cost_per_1k must be >= 0, got {self.max_cost_per_1k}"
            )
...
class EndpointSpec:
    ...
    def __post_init__(self) -> None:
        if not self.endpoint_id or not isinstance(self.endpoint_id, str):
            raise ValueError(
                f"endpoint_id must be non-empty str, got {self.endpoint_id!r}"
            )
        if not self.model_id or not isinstance(self.model_id, str):
            raise ValueError(
                f"model_id must be non-empty str, got {self.model_id!r}"
            )
        if self.cost_per_1k_input < 0:
            raise ValueError(
                f"cost_per_1k_input must be >= 0, got {self.cost_per_1k_input}"
            )
        if self.cost_per_1k_output < 0:
            raise ValueError(
                f"cost_per_1k_output must be >= 0, got {self.cost_per_1k_output}"
            )
        if self.avg_latency_ms < 0:
            raise ValueError(
                f"avg_latency_ms must be >= 0, got {self.avg_latency_ms}"
            )
        if self.tier not in _TIER_ORDER:
            raise ValueError(
                f"tier must be one of {_TIER_ORDER}, got {self.tier!r}"
            )
...
def select_endpoint(
    ...
    if not candidates and needs_vision:
        ...
        if candidates:
            ...
            vision_alt = _find_vision_alternative(
                endpoints, ctx, exclude_id=None
            )
