...
def _update_rating(rating: float, expected: float, actual: float, k: float) -> float:
    """Elo 评分更新
...
class EloLeaderboard:
    ...
    def _ensure_known(self, model_id: str) -> None:
        ...
        if model_id not in self._ratings:
            self._ratings[model_id] = EloRating(model_id=model_id, rating=1500.0, matches_played=0)
...
def to_json(obj, indent: int | None = 2) -> str:
    ...
    if isinstance(obj, (list, tuple)):
        return json.dumps(list(obj), indent=indent, ensure_ascii=False, default=_json_default)
    return json.dumps(obj, indent=indent, ensure_ascii=False, default=_json_default)
