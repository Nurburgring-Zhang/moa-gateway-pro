...
class LayerResult:
    ...
    )  # 该层喂进 aggregator 的 reference 文本(同 proposals + prev)
...
class MoAConfig:
    ...
    def __post_init__(self):
        ...
        if self.proposers_per_layer < 1:
            raise ValueError(f"proposers_per_layer must be >= 1, got {self.proposers_per_layer}")
...
def _format_proposals_for_aggregator(
    proposals: list[str], prev_aggregated: str | None = None
) -> str:
...
def _build_proposer_messages(
    query: str, proposer: Proposer, prev_aggregated: str | None = None
) -> list[dict]:
...
async def synthesize_layer(
    ...
    if not non_fallback:
        raise MoARunError(f"layer {layer_idx}: all {len(proposers)} proposers failed")
    ...
    try:
        ...
        logger.warning(
            "[n_layer_moa] aggregator %s failed, fallback to concat: %s", aggregator.name, e
        )
        aggregated = "\n\n---\n\n".join(non_fallback)
...
async def run_n_layer_moa(
    ...
    for i in range(1, config.num_layers + 1):
        ...
        if budget and budget > 0 and tokens_used >= budget:
            logger.info(
                "[n_layer_moa] budget hit after layer %d (%d/%d tokens)", i, tokens_used, budget
            )
            break
