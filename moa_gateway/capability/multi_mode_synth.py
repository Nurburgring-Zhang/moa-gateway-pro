...
def final_selection(
    ...
    # 按 score 降序排
    ranked: list[tuple[int, float]] = sorted(scores.items(), key=lambda x: -x[1])
...
def cross_iteration(
    ...
    if prev_best is not None:
        ...
        lines.append(
            f"prev_best: idx={prev_best.proposal_idx} score={_score_prop(prev_best):.2f} | {snippet}"
        )
    if curr_best is not None:
        snippet = curr_best.text[:80] + ("..." if len(curr_best.text) > 80 else "")
        lines.append(
            f"curr_best: idx={curr_best.proposal_idx} score={_score_prop(curr_best):.2f} | {snippet}"
        )
