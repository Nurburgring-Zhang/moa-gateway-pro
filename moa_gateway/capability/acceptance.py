...
def validate_pattern(ac: AcceptanceCriterion) -> str:
    ...
    if w and not g and not t:
        return (
            GEARSPattern.WHEN_ABNORMAL.value if _has_abnormal(w) else GEARSPattern.WHEN_NORMAL.value
        )
    if g and not w and not t:
        return (
            GEARSPattern.GIVEN_ABNORMAL.value
            if _has_abnormal(g)
            else GEARSPattern.GIVEN_NORMAL.value
        )
