...
class EndpointState:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cooldown_until is not None and self.cooldown_until < 0:
            raise ValueError(f"cooldown_until must be >= 0 if set, got {self.cooldown_until}")
...
class HealState:
    ...
    def __post_init__(self) -> None:
        if self.failure_threshold <= 0:
            raise ValueError(f"failure_threshold must be > 0, got {self.failure_threshold}")
...
def check_recovery(
    ...
    if ep.cooldown_until is None or ep.cooldown_until > now:
        return HealAction(
            action="no_op",
            endpoint_id=endpoint_id,
            reason=(f"cooldown not expired: cooldown_until={ep.cooldown_until} > now={now}"),
            at=now,
        )
