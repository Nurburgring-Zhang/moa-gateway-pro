...
_DEFAULT_DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rag_cache.sqlite3")
...
def _rank(
    corpus: list[dict[str, str]], query_tokens: list[str], max_results: int
) -> list[dict[str, Any]]:
...
def _cache_get(query_hash: str, ttl_hours: int) -> list[dict[str, Any]]:
    try:
        ...
        with _db_lock:
            ...
            try:
                row = conn.execute(
                    "SELECT results_json, created_at FROM rag_cache WHERE query_hash = ?",
                    (query_hash,),
                ).fetchone()
            finally:
                conn.close()
