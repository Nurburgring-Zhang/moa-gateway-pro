...
class StreamingProviderProtocol(Protocol):
    ...
    def chat_stream(self, prompt: str, model: str = "") -> AsyncIterator[StreamChunk]: ...
...
class MockStreamingProvider:
    ...
    def __init__(self, fail_prob: float = 0.1, seed: int | None = None) -> None:
    ...
    async def _stream_impl(self, prompt: str, model: str) -> AsyncIterator[StreamChunk]:
...
async def aggregate_stream(provider: Any, prompt: str, model: str = "") -> StreamResult:
    ...
    try:
            ...
            if chunk.delta_type == "text":
                ...
                if chunk.tool_call_id and chunk.tool_call_id in tool_calls_map:
                    if chunk.tool_call_args_delta:
                        tool_calls_map[chunk.tool_call_id]["function"]["arguments"] += (
        ...
        logger.warning("aggregate_stream: StreamError after %d chunks: %s", len(result.chunks), e)
        result.streaming_succeeded = False
    except Exception as e:  # noqa: BLE001
        logger.warning(
            "aggregate_stream: unexpected error after %d chunks: %s", len(result.chunks), e
        )
        result.streaming_succeeded = False
    else:
...
async def aggregate_with_fallback(provider: Any, prompt: str, model: str = "") -> StreamResult:
    ...
    try:
        ...
        logger.error("aggregate_with_fallback: non-stream fallback also failed: %s", e)
