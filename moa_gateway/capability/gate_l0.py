...
def handle_unit_convert(query: str) -> tuple[bool, str, str]:
    ...
    raw_value = m.group(1)
    # temperature
    if from_u in ("c", "celsius", "f", "fahrenheit", "k", "kelvin") and to_u in (
        "c",
        "celsius",
        "f",
        "fahrenheit",
...
MECHANICAL_PATTERNS: list[tuple[str, str, Callable | None]] = [
    ...
    (
        r"^[\d\s\+\-\*\/\.\(\)]+$|^(?:calculate|compute|evaluate|what(?:'s| is)?)\s+[\d\(]",
        "arithmetic",
        _handle_arithmetic,
    ),
    (
        r"today|now|current (?:date|time|year)|what\s+(?:day|date|year)",
        "datetime",
        _handle_datetime,
    ),
