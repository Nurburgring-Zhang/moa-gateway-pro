...
class PolicyRule:
    ...
    def from_dict(cls, d: dict) -> PolicyRule:
        ...
        if "name" in d:
            kwargs["name"] = d["name"]
        if "name" not in kwargs and "pattern" in d:
            kwargs["name"] = d["pattern"]
        if "action" in d:
            kwargs["action"] = d["action"]
        if "action" not in kwargs and "action" in d:
            kwargs["action"] = d["action"]
        if "pattern" in d:
            kwargs["pattern"] = d["pattern"]
        if "pattern" not in kwargs and "name" in d:
            kwargs["pattern"] = d["name"]
        if "match_type" in d:
            kwargs["match_type"] = d["match_type"]
        if "match_type" not in kwargs and "priority" in d:
            kwargs["match_type"] = d["priority"]
        if "reason" in d:
            kwargs["reason"] = d["reason"]
        if "reason" not in kwargs and "priority" in d:
            kwargs["reason"] = d["priority"]
        return cls(**kwargs)
