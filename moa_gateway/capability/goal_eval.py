...
def evaluate_tier1(goal: Goal, output: Any) -> GoalResult:
    ...
    if goal.tier != GoalTier.MECHANICAL:
        raise ValueError(f"evaluate_tier1 requires MECHANICAL goal, got {goal.tier}")
...
def evaluate_tier2(
    ...
    if goal.tier != GoalTier.MODEL_DECLARED:
        raise ValueError(f"evaluate_tier2 requires MODEL_DECLARED goal, got {goal.tier}")
    ...
    if model_call is not None:
        try:
            ...
            # 其它类型 → fallback
            evidence.append(
                f"model_call returned unsupported type {type(model_out).__name__}, fallback to heuristic"
            )
        except Exception as e:
