...
_ACT_PATTERNS: Tuple[Tuple[str, str], ...] = (
    ...
    (r"act_lets",        r"\b(?:let'?s|lets)\s+(?:run|execute|build|deploy|install|click|type|paste|search|fetch|download|edit|fix|create|make|send|write|open|do)\b"),
    (r"act_go_ahead",    r"\bgo\s+ahead\s+(?:and|to)\b"),
    (r"act_please_verb", r"^\s*please\s+(?:run|execute|build|deploy|install|click|type|paste|search|fetch|download|edit|fix|create|make|send|write|open)\b"),
