...
class ModelEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.context_window, int) or self.context_window < 0:
            raise ValueError(
                f"context_window must be int >= 0, got {self.context_window!r}"
            )
        if not isinstance(self.max_output, int) or self.max_output < 0:
            raise ValueError(
                f"max_output must be int >= 0, got {self.max_output!r}"
            )
        ...
        if not isinstance(self.modalities, list):
            raise ValueError(
                f"modalities must be list, got {type(self.modalities).__name__}"
            )
        for m in self.modalities:
            if not isinstance(m, Modality):
                raise ValueError(
                    f"modalities entries must be Modality, got {type(m).__name__}"
                )
        for bname in (
            "supports_tools",
            "supports_vision",
        ...
        if self.input_cost_per_1k < 0.0:
            raise ValueError(
                f"input_cost_per_1k must be >= 0, got {self.input_cost_per_1k}"
            )
...
def filter_by_capability(
    ...
    if capability not in _CAPABILITY_FIELDS:
        raise ValueError(
            f"capability must be one of {sorted(_CAPABILITY_FIELDS)}, "
            f"got {capability!r}"
        )
...
def filter_by_modality(
    ...
    if not isinstance(modality, Modality):
        raise ValueError(
            f"modality must be Modality enum, got {type(modality).__name__}"
        )
...
def sort_by_cost(
    ...
    if cost_field not in ("input_cost_per_1k", "output_cost_per_1k"):
        raise ValueError(
            f"cost_field must be input_cost_per_1k or output_cost_per_1k, "
            f"got {cost_field!r}"
        )
...
def from_json(data: str) -> List[ModelEntry]:
    ...
    for item in raw:
        if not isinstance(item, dict):
            raise ValueError(
                f"each entry must be a dict, got {type(item).__name__}"
            )
        ...
        for m in modalities_raw:
            if isinstance(m, Modality):
                ...
                raise ValueError(
                    f"modality entry must be str or Modality, got {type(m).__name__}"
                )
        item = dict(item)
