...
def is_exempt(file: str, line: int, line_text: str, all_lines: List[str],
    ...
    for kw in ("exempt", "fixture", "example", "mock", "test_data", ".template",
               "test_", "/test", "tests/", "_test.",
               "参考", "分析"):  # 参考项目 + analysis docs
        if kw in path_lower:
...
DEFAULT_SKIP_DIRS = frozenset({
    ...
    "dist", "build", ".pytest_cache", "data", "data.bak", "extracted",
...
def _is_text_file(p: Path) -> bool:
    ...
    if p.suffix.lower() in {".png", ".jpg", ".jpeg", ".gif", ".zip", ".tar", ".gz",
                              ".exe", ".dll", ".so", ".dylib", ".pdf", ".mp4",
                              ".mp3", ".woff", ".woff2", ".ttf", ".ico"}:
        return False
    try:
        with p.open("rb") as f:
            chunk = f.read(8192)
...
def scan_path(
    ...
    if should_block(result, fail_on=3):
        ...
        result.block_reasons.append(
            f"found {result.real_count} real secrets (severity >= 3)"
        )
    return result
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.json:
        ...
        for f in result.findings:
            ...
            print(f"  {tag} {f.pattern_id} {f.file}:{f.line}:{f.column} → {f.redacted}"
