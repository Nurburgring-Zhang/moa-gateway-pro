...
def score_option(option: ConflictOption, total_proposals: int = 0) -> dict[str, float]:
    ...
    total = v * WEIGHT_VIABILITY + s * WEIGHT_SUPPORT + e * WEIGHT_EMPIRICAL + c * WEIGHT_COMPILABLE
...
def _rationale(
    ...
    parts: list[str] = []
    parts.append(f"选项 {winner_id} ({winner_desc}) 以总分 {winner_scores['total']:.3f} 胜出")
    # 列出 top-2 维度
    dims = [
        ("viability", "可行性"),
        ("support", "支持率"),
    ...
    if runner_id and runner_scores:
        parts.append(f"次优 {runner_id} 总分 {runner_scores['total']:.3f}")
    return "; ".join(parts)
...
def arbitrate(
    ...
    scored.sort(key=lambda x: (-x[1]["total"], -x[1]["support"], x[0].option_id))
