...
def merge_consecutive_role(messages: List[Message], role: Role) -> List[Message]:
    ...
    for m in messages:
        ...
        if out and out[-1].role == role:
            ...
            new_tool_call_id = prev.tool_call_id if prev.tool_call_id is not None else m.tool_call_id
...
def _strip_orphan_tool_messages(messages: List[Message]) -> Tuple[List[Message], int]:
    ...
    for i, m in enumerate(messages):
        ...
        if m.tool_call_id is not None:
            ids = {tc.get("id") for tc in (parent_assistant.tool_calls or []) if isinstance(tc, dict)}
...
def from_openai_format(data: List[Dict]) -> List[Message]:
    ...
    for item in data:
        ...
        out.append(Message(
            ...
            tool_calls=copy.deepcopy(item.get("tool_calls")) if item.get("tool_calls") is not None else None,
        ))
    return out
...
if __name__ == "__main__":
    sample = [
        ...
        Message("assistant", "done", tool_calls=[{"id": "x", "type": "function", "function": {"name": "f", "arguments": "{}"}}]),
        Message(""),
    ]
