...
class Channel:
    ...
    def from_dict(cls, d: dict) -> Channel:
        ...
        if "name" in d:
            kwargs["name"] = d["name"]
        if "name" not in kwargs and "name" in d:
            kwargs["name"] = d["name"]
        if "cost_per_1k_input" in d:
            kwargs["cost_per_1k_input"] = d["cost_per_1k_input"]
        if "cost_per_1k_input" not in kwargs and "cost_per_1k_input" in d:
            kwargs["cost_per_1k_input"] = d["cost_per_1k_input"]
        if "cost_per_1k_output" in d:
            kwargs["cost_per_1k_output"] = d["cost_per_1k_output"]
        if "cost_per_1k_output" not in kwargs and "cost_per_1k_output" in d:
            kwargs["cost_per_1k_output"] = d["cost_per_1k_output"]
        if "avg_latency_ms" in d:
            kwargs["avg_latency_ms"] = d["avg_latency_ms"]
        if "avg_latency_ms" not in kwargs and "tier" in d:
            kwargs["avg_latency_ms"] = d["tier"]
        if "reliability" in d:
            kwargs["reliability"] = d["reliability"]
        if "reliability" not in kwargs and "tier" in d:
            kwargs["reliability"] = d["tier"]
        return cls(**kwargs)
...
def estimate_moa_cost(
    ...
    for ch in channels:
        ...
        notes.append(f"ref {ch.name}: {input_tokens}in + {output_tokens}out = ${ch_cost:.6f}")
...
def format_report(estimate: CostEstimate) -> str:
    ...
    lines.append(f"Total: ${estimate.total_cost_usd:.4f} (multiplier {estimate.multiplier:.2f}x)")
