...
def build_benchmark(state: Dict, sr) -> ft.Control:
        ...
        for i, b in enumerate(seg_btns):
            if i == idx:
                b.style = ft.ButtonStyle(
                    ...
                    color=palette.text, shape=ft.RoundedRectangleBorder(radius=7),
                )
            else:
                b.style = ft.ButtonStyle(
                    bgcolor=None, color=palette.text_sec,
    ...
    for i, label in enumerate(["📋 Suite", "📈 Pareto", "🌊 Layered", "🎯 FLASK"]):
        b = ft.TextButton(
            label, on_click=lambda _, idx=i: switch_tab(idx),
        ...
        if i == 0:
            b.style = ft.ButtonStyle(
                bgcolor=DARK.bg_tertiary, color=palette.text,
                shape=ft.RoundedRectangleBorder(radius=7),
            )
    ...
    spin_limit = ft.TextField(label="每类题数", value="2", width=120, keyboard_type=ft.KeyboardType.NUMBER)
    ...
    suite_table = ft.DataTable(
        ...
        rows=[],
        border=ft.border.all(1, palette.border), border_radius=10,
    )

    def run_suite(e):
        cats = []
        for cb, k in [(cb_reasoning, "reasoning"), (cb_code, "code"),
                      (cb_chinese, "chinese"), (cb_creative, "creative"),
                      (cb_pro, "professional")]:
            if cb.value:
                cats.append(k)
        if not cats:
        ...
        async def do():
            try:
                return await http_post(f"{base}/v1/moa/benchmark",
        ...
        rows = []
        for p in prompts:
            cells = [ft.DataCell(ft.Text(f"[{p['category']}] {p['text'][:50]}...", color=palette.text))]
            for preset in preset_names:
                ...
                if cell:
                    ...
                    color = palette.success if score >= 80 else (palette.warning if score >= 60 else palette.text_dim)
    ...
    suite_toolbar = ft.Row(
        controls=[
            cb_reasoning, cb_code, cb_chinese, cb_creative, cb_pro,
            spin_limit,
    ...
    pareto_presets = ft.TextField(
        label="preset 列表(逗号)", value="fast,balanced,quality,chinese_battalion",
        width=400,
    )
    ...
    pareto_image = ft.Container(
        ...
        alignment=ft.alignment.center, height=400,
        bgcolor=palette.bg_tertiary if hasattr(palette, "bg_tertiary") else DARK.bg_tertiary,
        border_radius=10,
    )
    ...
    def run_pareto(e):
        ...
        async def do():
            try:
                return await http_post(f"{base}/v1/moa/cost-pareto",
        ...
        pareto_summary.value = f"Pareto 前沿: {' → '.join(frontier) or '—'}    推荐: {recommended or '—'}"
        # 画 SVG
    ...
    def run_layered(e):
        ...
        async def do():
            try:
                return await http_post(f"{base}/v1/chat/completions", {
                    "model": "moa",
        ...
        if data is None:
            data = {"error": "call_async timeout/failed"}
        if "error" in data:
            layered_result.content = f"<div style='color:#ff453a;padding:20px;'>❌ {data['error']}</div>"
        ...
        layered_result.content = ft.Container(
            content=ft.Column(
                controls=[
                    ...
                    ft.Text(f"tokens: {usage.get('total_tokens', 0)}  ·  preset: {preset}",
    ...
    sr.on_started_callbacks.append(lambda: __import__('threading').Thread(target=load_layered_presets, daemon=True).start())

    layered_toolbar = ft.Row(
        controls=[
            layered_preset, layered_query,
    ...
    flask_response = ft.TextField(
        label="回答", multiline=True, min_lines=4, value=(
    ...
    def run_flask(e):
        ...
        async def do():
            try:
                return await http_post(f"{base}/v1/moa/flask", {
                    "query": flask_query.value, "response": flask_response.value,
                }, timeout=120, token=sr.admin_token)
            except Exception as ex:
                return {"error": str(ex)}
        data = sr.call_async(do(), timeout=120)
        if data is None:
            data = {"error": "call_async timeout/failed"}
        if "error" in data:
            flask_result.content = f"<div style='color:#ff453a;padding:20px;'>❌ {data['error']}</div>"
            flask_result.update()
            return
        ...
        if not scores:
            flask_result.content = ft.Column(
                controls=[
                    ft.Text(f"Judge 模型未返回有效 JSON。Avg: {avg}", size=12, color=palette.text_sec),
                    ft.Text(data.get('raw_response', '')[:500], size=11, color=palette.text_dim,
                            font_family="JetBrains Mono, monospace"),
                ], spacing=8,
            )
            flask_result.update()
        ...
        for dim, v_ in scores.items():
            ...
            score_rows.append(ft.Container(
                content=ft.Row(
                    controls=[
                        ft.Text(dim, size=12, color=palette.text, width=120),
                        ft.Text(f"{s}/5", size=12, weight=ft.FontWeight.BOLD, color=color, width=40),
            ...
            controls=[
                ...
                ft.Column(
                    controls=[
                        ft.Row(
                            controls=[
                                ft.Text(f"{avg:.1f}", size=32, weight=ft.FontWeight.BOLD, color=palette.text),
                                ft.Text("/ 100", size=14, color=palette.text_dim),
                            ], spacing=4, vertical_alignment=ft.CrossAxisAlignment.END,
                        ),
                        ft.Text(f"Judge: {data.get('judge_model', '?')}", size=11, color=palette.text_dim),
                        ft.Container(content=ft.Column(controls=score_rows, spacing=0),
                                      margin=ft.margin.only(top=12), expand=True),
    ...
    flask_toolbar = ft.Column(
        controls=[flask_query, flask_response, primary_button(palette, "🎯 FLASK 12 维评分", run_flask)],
        spacing=10,
    )
    ...
    seg = ft.Container(
        ...
        padding=3, border_radius=9,
        bgcolor=DARK.bg_tertiary, border=ft.border.all(1, palette.border),
    )
    seg_wrap = ft.Container(content=seg, padding=ft.padding.symmetric(horizontal=32, vertical=8))

    return ft.Column(
        controls=[
            page_header(palette, "Benchmark 可视化", "Suite 跑分 / Cost Pareto / Layered 流程 / FLASK 雷达"),
            seg_wrap,
            ft.Container(content=stack, padding=ft.padding.symmetric(horizontal=32, vertical=8), expand=True),
...
def draw_pareto_svg(points, frontier, recommended, palette) -> str:
    ...
    def to_x(c): return px + (c - min_c) / max(1e-6, (max_c - min_c)) * iw
    def to_y(s): return py + ih - (s - min_s) / max(1e-6, (max_s - min_s)) * ih

    svg = f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg">'
    for i in range(0, 6):
        yy, xx = py + i * ih / 5, px + i * iw / 5
        svg += f'<line x1="{px}" y1="{yy}" x2="{w-30}" y2="{yy}" stroke="rgba(128,128,128,0.2)"/>'
        svg += f'<line x1="{xx}" y1="{py}" x2="{xx}" y2="{py+ih}" stroke="rgba(128,128,128,0.2)"/>'
    ...
    if len(frontier) > 1:
        front_pts = sorted([p for p in points if p["preset"] in frontier], key=lambda p: p["avg_cost"])
        path = "M " + " L ".join(f"{to_x(p['avg_cost']):.1f} {to_y(p['avg_score']):.1f}" for p in front_pts)
...
def build_prompts(state: Dict, sr) -> ft.Control:
    ...
    editor = ft.TextField(
        label="模板内容", multiline=True, min_lines=20, read_only=False,
        text_size=12,
    )
    ...
    def on_save(e):
        ...
        async def do():
            try:
                return await http_post(f"{base}/v1/moa/prompts/{current_name[0]}",
                    {"content": editor.value or ""}, timeout=10, token=sr.admin_token)
            except Exception as ex:
                return {"error": str(ex)}
        data = sr.call_async(do(), timeout=10)
        if data is None:
        ...
        if "error" in data:
            ...
            if page:
                page.snack_bar = ft.SnackBar(content=ft.Text(f"❌ {data['error']}"), bgcolor=palette.danger)
                page.snack_bar.open = True
                page.update()
        else:
            page = state.get("page_ref")
            if page:
                page.snack_bar = ft.SnackBar(content=ft.Text(f"✓ 已保存 {current_name[0]}"), bgcolor=palette.success)
                page.snack_bar.open = True
                page.update()
            load_templates()
    ...
    sr.on_started_callbacks.append(lambda: __import__('threading').Thread(target=load_templates, daemon=True).start())
    ...
    left = ft.Container(
        ...
        width=300, padding=ft.padding.all(12),
        bgcolor=palette.card, border_radius=14,
    ...
    right = ft.Container(
        content=ft.Column(
            controls=[
                ft.Row(
                    controls=[lbl_name, lbl_meta, ft.Container(expand=True),
                              primary_button(palette, "💾 保存", on_save),
                              ft.ElevatedButton("🗑️ 删除", bgcolor=palette.danger, color="#ffffff",
                                                on_click=on_delete)],
    ...
    return ft.Column(
        controls=[
            ...
            ft.Container(
                ...
                padding=ft.padding.symmetric(horizontal=32, vertical=8), expand=True,
            ),
        ], spacing=0, expand=True,
    )
...
def build_settings(state: Dict, sr, apply_theme, toggle_server, server_runner) -> ft.Control:
    ...
    def change_pw(e):
        ...
        def do_save(_):
            ...
            async def do():
                try:
                    return await http_post(f"http://127.0.0.1:{sr.port or 8765}/api/auth/change-password",
            ...
            if "error" in data:
                ...
                page.close(dlg)
                if page:
                    page.snack_bar = ft.SnackBar(content=ft.Text("✓ 密码已修改"), bgcolor=palette.success)
                    page.snack_bar.open = True
                    page.update()
        dlg = ft.AlertDialog(
            modal=True, title=ft.Text("修改管理员密码"),
            ...
            actions=[
                ...
                ft.ElevatedButton("保存", bgcolor=palette.accent, color="#ffffff",
                                  on_click=do_save),
            ],
        )
        if page:
    ...
    about_card = ft.Container(
        content=ft.Column(
            controls=[
                ...
                ft.Text("🏗️ 9 种 MoA strategy  ·  12 preset  ·  16 模型端点  ·  12 维 FLASK  ·  7 adapter",
                        size=11, color=palette.text_sec),
                ft.Text("💡 对齐 Together AI MoA + OpenSquilla + Hermes 委员会", size=11, color=palette.text_sec),
                ft.Text("⚖️ MIT License  ·  跨平台 (Windows / macOS / Linux)", size=11, color=palette.text_sec),
            ], spacing=6,
        ),
        bgcolor=palette.card, border_radius=14,
        border=ft.border.all(1, palette.border),
    ...
    body = ft.Column(
        controls=[
            ...
            ft.Container(
                content=ft.Column(
                    controls=[theme_card, server_card, about_card], spacing=12,
                ),
