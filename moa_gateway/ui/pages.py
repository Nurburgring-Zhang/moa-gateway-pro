            ...
            ft.Column(
                controls=[
                    ft.Text(title, size=16, weight=ft.FontWeight.BOLD,
                            color=palette.text) if title else ft.Container(),
                    ft.Text(sub, size=11, color=palette.text_dim) if sub else ft.Container(),
                ], spacing=2,
            ),
        ],
    ) if title or sub else None
...
def primary_button(palette, label: str, on_click=None) -> ft.ElevatedButton:
    return ft.ElevatedButton(
        label, bgcolor=palette.accent, color="#ffffff",
...
def badge(palette, text: str, kind: str = "info") -> ft.Container:
    ...
    return ft.Container(
        ...
        bgcolor=bg, border_radius=6, padding=ft.padding.symmetric(horizontal=8, vertical=2),
    )
...
def build_dashboard(state: Dict, sr) -> ft.Control:
    ...
    def refresh(e=None):
        ...
        for ep in eps[:20]:
            ...
            h_color = palette.success if h == "healthy" else (palette.danger if h == "unhealthy" else palette.text_dim)
            rows.append(ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Text(f"{ep.get('avg_latency_ms', 0):.0f}ms", color=palette.text)),
            ]))
        ep_table.rows = rows
        ep_table.update()

    # 启动时自动刷新
    sr.on_started_callbacks.append(lambda: __import__('threading').Thread(target=refresh, daemon=True).start())
        ...
        content=ft.Column(
            controls=[
                ...
                ft.GridView(
                    ...
                    runs_count=2, max_extent=200, child_aspect_ratio=2.5, spacing=8,
                ),
            ], spacing=8,
        ),
        bgcolor=palette.card, border_radius=14,
...
def build_endpoints(state: Dict, sr) -> ft.Control:
        ...
        for i, ep in enumerate(endpoints_data):
            ...
            actions = ft.Row(
                controls=[
                    ...
                    ghost_button(palette, "停用" if enabled else "启用", lambda _, idx=i: toggle(idx)),
                ], spacing=4,
            )
            rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(ep.get("id", ""), color=palette.text)),
                ...
                ft.DataCell(ft.Text("✓" if enabled else "✗",
    ...
    def open_edit(idx: Optional[int]):
        ...
        provider_e = ft.Dropdown(
            label="Provider", value=(ep.get("provider", "openai") if ep else "openai"),
            options=[ft.dropdown.Option(p) for p in [
                "openai", "anthropic", "deepseek", "zhipu", "moonshot",
                "qwen", "doubao", "lingyi", "baichuan", "mistral", "google", "mock",
            ]],
            width=400,
        )
        model_e = ft.TextField(value=ep.get("model", "") if ep else "", label="Model", width=400)
        api_base_e = ft.TextField(value=ep.get("api_base", "") if ep else "", label="API Base", width=400)
        api_key_e = ft.TextField(value=ep.get("api_key", "") if ep else "", label="API Key / Env", password=True, width=400)
        tier_e = ft.Dropdown(
            label="Tier", value=(ep.get("tier", "standard") if ep else "standard"),
            options=[ft.dropdown.Option(t) for t in ["free", "lite", "standard", "premium", "flagship"]],
            width=400,
        )
        ...
        def save(close_dlg):
            ...
            async def do():
                try:
                    if is_new:
                        return await http_post(f"{base}/api/endpoints", payload, timeout=15, token=sr.admin_token)
                    else:
                        return await http_post(f"{base}/api/endpoints/{ep['id']}", payload, timeout=15, token=sr.admin_token)
                except Exception as ex:
                    return {"error": str(ex)}
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Container(
                content=ft.Column(
                    controls=[
                        id_e, name_e, provider_e, model_e,
                        api_base_e, api_key_e, tier_e, enabled_e, msg,
    ...
    def toggle(idx):
        ...
        async def do():
            try:
                return await http_post(f"{base}/api/endpoints/{eid}/toggle", {}, token=sr.admin_token)
            except Exception as ex:
                return {"error": str(ex)}
...
def build_playground(state: Dict, sr) -> ft.Control:
    ...
    preset_dd = ft.Dropdown(
        label="Preset", options=[], width=320, value=None,
    )
    strategy_desc = ft.Text("—", size=11, color=palette.text_dim)
    spin_n = ft.TextField(label="参考模型数", value="3", width=320, keyboard_type=ft.KeyboardType.NUMBER)
    spin_tref = ft.TextField(label="参考温度", value="0.6", width=320, keyboard_type=ft.KeyboardType.NUMBER)
    spin_tagg = ft.TextField(label="聚合温度", value="0.4", width=320, keyboard_type=ft.KeyboardType.NUMBER)
    spin_crit = ft.TextField(label="Critic 轮数", value="1", width=320, keyboard_type=ft.KeyboardType.NUMBER)
    spin_layer = ft.TextField(label="层数 (layered)", value="3", width=320, keyboard_type=ft.KeyboardType.NUMBER)

    txt_input = ft.TextField(
        label="输入问题", multiline=True, min_lines=3, max_lines=5,
        value="用 Python 写一个 LRU Cache,要求 O(1) 复杂度。",
        text_size=13,
    )
    txt_output = ft.TextField(
        label="输出结果", multiline=True, min_lines=10, read_only=True,
        value="点击运行查看结果…",
        text_size=12,
    )
    ...
    def load_presets(e=None):
        ...
        preset_dd.options = [
            ft.dropdown.Option(p["name"], f"{p['name']} · {p['strategy']}"
    ...
    def on_run(e):
        ...
        async def do():
            try:
                return await http_post(f"{base}/v1/chat/completions", payload, timeout=180, token=sr.admin_token)
            except Exception as ex:
                return {"error": str(ex)}
    ...
    sr.on_started_callbacks.append(lambda: __import__('threading').Thread(target=load_presets, daemon=True).start())
    ...
    left = ft.Container(
        content=ft.Column(
            controls=[
                ...
                spin_n, spin_tref, spin_tagg, spin_crit, spin_layer,
        ...
        width=360, padding=ft.padding.all(20),
        bgcolor=palette.card, border_radius=14,
        border=ft.border.all(1, palette.border),
    )
