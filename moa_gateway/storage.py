...
def _bcrypt_verify(password: str, hashed: str) -> bool:
    if not hashed:
        logger.error("Attempt to verify against empty password hash")
        return False
...
def _get_or_create_fernet() -> Fernet:
    ...
    with _FERNET_SINGLEFLIGHT:
        ...
        if _FERNET_PATH.exists():
            ...
            try:
                fd = os.open(str(_FERNET_PATH), os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
                try:
                    os.write(fd, key)
                finally:
                    os.close(fd)
...
class Storage:
    ...
    def conn(self) -> Iterator[sqlite3.Connection]:
        """Get database connection - auto-selects SQLite/PostgreSQL backend"""
    ...
    def get_conn(self) -> sqlite3.Connection:
        ...
        if self._persistent_conn is None:
            ...
            # Apply PRAGMA optimizations via the SQLite backend
            self._engine.backend._apply_pragmas(self._persistent_conn)
    ...
    def _bootstrap_admin(self, settings: Settings) -> None:
        ...
        with self.conn() as c:
            row = c.execute(
                "SELECT id FROM admin_users WHERE username = ?", (settings.auth.admin_username,)
            ).fetchone()
            if not row:
    ...
    def verify_admin(self, username: str, password: str) -> dict[str, Any] | None:
        ...
        with self.conn() as c:
            ...
            must_change = (
                ...
                and (
                    password.lower() == settings.auth.admin_password.lower()
                    or password.lower() in WEAK_PASSWORDS  # WEAK_PASSWORDS已经全小写
                )
    ...
    def create_api_key(
        self, name: str, quota_rpm: int = 60, quota_daily_tokens: int = 5_000_000
    ) -> dict[str, Any]:
        ...
        with self.conn() as c:
            c.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, 1, ?)",
                (key_id, self._hash_key(raw), name, quota_rpm, quota_daily_tokens, time.time()),
            )
        return {
            "key_id": key_id,
            "key": raw,
    ...
    # SEC-001: 白名单 — 防止通过动态列名拼接实现 SQL 注入
    ALLOWED_API_KEY_FIELDS = {
        "name",
        "quota_rpm",
        "quota_daily_tokens",
        "enabled",
    ...
    def update_api_key(self, key_id: str, **fields) -> bool:
        if not fields:
            return False
        for k in fields:
            if k not in self.ALLOWED_API_KEY_FIELDS:
    ...
    def find_api_key(self, raw: str) -> dict[str, Any] | None:
        ...
        with self.conn() as c:
            row = c.execute(
                "SELECT * FROM api_keys WHERE key_hash = ? AND enabled = 1", (h,)
            ).fetchone()
            if not row:
                return None
            d = dict(row)
            c.execute(
                "UPDATE api_keys SET last_used = ? WHERE key_id = ?", (time.time(), d["key_id"])
            )
            return d
    ...
    # SEC-001: 白名单 — 限定 upsert_endpoint 可更新的列
    ...
    def upsert_endpoint(self, ep: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.conn() as c:
            row = c.execute(
                "SELECT id FROM model_endpoints WHERE endpoint_id = ?", (eid,)
            ).fetchone()
            if row:
                updates = []
                args = []
                for k in [
                    ...
                    if k not in self.ALLOWED_ENDPOINT_FIELDS:
                        raise ValueError(f"Invalid field for endpoint update: {k}")
                ...
                args.append(eid)
                c.execute(
                    f"UPDATE model_endpoints SET {', '.join(updates)} WHERE endpoint_id = ?", args
                )
            else:
                c.execute(
    ...
    def get_endpoint(self, eid: str) -> dict[str, Any] | None:
        with self.conn() as c:
            row = c.execute(
                "SELECT * FROM model_endpoints WHERE endpoint_id = ?", (eid,)
            ).fetchone()
            if not row:
                return None
            d = dict(row)
    ...
    def list_logs(self, limit: int = 100, api_key_id: str | None = None) -> list[dict[str, Any]]:
        with self.conn() as c:
            if api_key_id:
                rows = c.execute(
                    "SELECT * FROM request_logs WHERE api_key_id = ? "
    ...
    def incr_rpm(self, api_key_id: str, bucket: str) -> int:
        """Atomic inc+select - prevents lost-update rate-limit bypass"""
        with self.conn() as c:
            if self._engine.is_sqlite:
                ...
                # PostgreSQL: ON CONFLICT is natively atomic
                c.execute(
                    "INSERT INTO ratelimit_buckets (api_key_id, bucket, count, updated_at) "
                    "VALUES (?, ?, 1, ?) "
                    "ON CONFLICT(api_key_id, bucket) DO UPDATE SET count = count + 1, updated_at = ?",
    ...
    def incr_daily_tokens(self, api_key_id: str, day: str, tokens: int) -> int:
        """Atomic inc+select - prevents lost-update"""
        with self.conn() as c:
            if self._engine.is_sqlite:
                c.execute("BEGIN IMMEDIATE")
                try:
                ...
                # PostgreSQL: ON CONFLICT natively atomic
                c.execute(
                    "INSERT INTO ratelimit_tokens (api_key_id, day, tokens) VALUES (?, ?, ?) "
    ...
    def create_admin_user(self, username: str, password: str, role: str = "user") -> dict | None:
        """Create a new admin user. Returns user dict or None if username exists."""
        valid_roles = {"admin", "operator", "user", "readonly"}
        ...
        with self.conn() as c:
            ...
            c.execute(
                "INSERT INTO admin_users (username, password_hash, role, created_at) "
                "VALUES (?, ?, ?, ?)",
                (username, h, role, __import__("time").time()),
            )
            row = c.execute(
                "SELECT id, username, role, created_at, last_login FROM admin_users WHERE username = ?",
                (username,),
